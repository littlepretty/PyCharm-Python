...
def resolve_resources(br, path, course):
    ...
    for l in br.links():
        ...
        if m_title:
            t = resolve_name_with_illegal_char(m_title.group(1).strip());
            t = "%03d_%s" % (num_title,t)
            num_title += 1
            title.append(t)
        if m_pdf:
            pdf.append(["%03d_%s" % (num_title-1, resolve_name_with_hex(m_pdf.group(1))), m_pdf.group()])
        if m_pptx:
    ...
    print len(title),len(link_srt)
    if len(title) == len(link_srt):
        srt = zip([t+'.srt' for t in title], link_srt)
    elif course == 'compdata-2012-001':
        srt = zip([t+'.srt' for t in title[:len(link_srt)]], link_srt) 
    else:
...
def main():
    ...
    course = { 
                ...
                '3' : 'sciwrite-2012-001',
                '4' : 'hci-2012-002',
                '5' : 'scientificcomp-2012-001',
                '6' : 'stats1-2012-001',
                '7' : 'neuralnets-2012-001'
            }

    # Your Coursera.org email and password needed here to download videos. 
    email = 'littlepretty881203@gmail.com'
    password = '1988102312'
